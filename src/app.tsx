...
const App: FunctionComponent = () => {
  ...
  const onSubmitPreRegister = async (formData: object, isValid: boolean) => {
    ...
    if (isValid) {
      const requestFunc = Config.demo
        ? API.postPreRegisterUserDataMock
        : API.postPreRegisterUserData;
      ...
      await requestFunc(formData)
        .then(() => {
          setFormAlert({
            ...
            message:
              "Félicitations ! Tu fais partie des futur·e·s testeur·euse·s de l'application Case Tes Potes !  La Team te tiendra informé·e bientôt de la disponibilité de l'application !",
          });
        ...
        .catch((e: AxiosError) => {
          if (e.response?.status === 400) {
            setFormAlert({
              ...
              message:
                "Tu es déjà préinscrit·e en tant que testeur·euse de l'application Case Tes Potes ! La Team te tiendra informé·e bientôt de la disponibilité de l'application !",
            });
          } else {
            setFormAlert({
              type: PopupAlert.alert,
  ...
  return (
    <div className={`background-app ${bgLight ? "background-app--light" : "background-app--dark"}`}>
      ...
      <MyRoute path={homepage.path} themeLight={homepage.themeLight} callbackScroll={handleScroll}>
        <Homepage
          onSubmitPreRegister={onSubmitPreRegister}
          formIsLoading={formIsLoading}
          formSubmitIsValid={formPreregisterSuccess}
        />
      </MyRoute>
      <MyRoute path={contact.path} themeLight={contact.themeLight} callbackScroll={handleScroll}>
        <Contact
          ...
          formIsLoading={formIsLoading}
          formSubmitIsValid={formContactSuccess}
        />
      </MyRoute>
      ...
      {locationError ? (
        <MyRoute
          path={error404.path}
          themeLight={error404.themeLight}
          callbackScroll={handleScroll}
        >
          <Error404 />
        </MyRoute>
      ) : null}
      <Smartphone
        ...
        locationsAllow={[RoutesLD.getRouteByName("home").path]}
      />
      <div className={`bg-full-width ${bgLoaded ? "bg-full-width--loaded" : ""}`}>
        <img
          ...
          width="500"
          src={background}
          alt="case tes potes lancement application"
        />
      </div>
    </div>
  );
